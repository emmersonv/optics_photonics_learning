...
$$\vec E = \frac{1}{j\omega\epsilon}\nabla \times \vec H = \frac{1}{j\omega\epsilon}\left(-j\beta H_x(y)e^{j(\omega t - \beta z)}\hat{y} - \frac{\partial H_x}{\partial z}e^{j(\omega t - \beta z)} \hat{z} \right) = -\frac{\beta}{\omega \epsilon}H_x(y)e$$
