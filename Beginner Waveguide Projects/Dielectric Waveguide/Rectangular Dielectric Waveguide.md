...
Starting with the Helmholtz equation for the magnetic field $$\nabla^2\vec H +k^2 \vec H = 0 \qquad...()$$
The field should propagate down the waveguide with some field profile in the $y$ direction **(add a photo)**, so the solution is assumed to be $$\vec H = \hat{x}H_y(y)e^{j(\omega t - \beta z)}$$
