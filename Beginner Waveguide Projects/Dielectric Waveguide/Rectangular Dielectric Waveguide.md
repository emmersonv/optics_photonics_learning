...
Calculating the cross product of the $H$ field results in $$\nabla \times \vec H = \frac{\partial H_x}{\partial z}\hat{y} - \frac{\partial H_x}{\partial y}\hat{z}=-j\beta H_x(y) e^{j(\omega t-\beta z)} \hat{y} - \frac{\partial H_x}{\partial z}e^{j(\omega t - \beta z)}$$
